...
class SpriteEditorApp(ctk.CTk):
    def __init__(self) -> None:
        ...
        # Ferramentas
        # ============================
        # pencil | eraser | fill | line | rect | rect_fill | ellipse | ellipse_fill
        self.tool_mode: str = "pencil"

        # borracha (forma/tamanho)
    ...
    def _flood_fill_from(self, start_x: int, start_y: int, new_value: int) -> int:
        """Preenche a região conectada (4-dir) a partir de (start_x, start_y).
        Regra: substitui pixels do mesmo valor inicial (target) por new_value.
        Retorna quantidade de pixels alterados.
        """
        ...
        while q:
            ...
            # pintar/apagar aqui
            if new_value == 1:
                self._apply_point_in_mode(x, y, 1, set_color=True)
            else:
                # apagar não altera a cor do sprite
                self._apply_point_in_mode(x, y, 0, set_color=False)
    ...
    # ----------------------------
    # UI layout (reorganizada)
    # ----------------------------
    def _build_layout(self) -> None:
        # Layout raiz em grid: topo + conteúdo
        ...
        self.mode_seg = ctk.CTkSegmentedButton(left2, values=["single", "2x2", "overlay"], command=lambda _: self._redraw_all(), width=260)
        ...
        # --- Coluna esquerda: tabela de sprites ---
        ...
        # --- Coluna direita: editor + painel inferior (aproveita espaço de baixo) ---
        ...
        # Linha 2: pincéis + borracha (opções) + ações de pincel
        ...
        # espaço reservado para futuros botões (ex.: flip, rotate, clear)
        self.btn_future2 = ctk.CTkButton(shift, text="(futuro)", width=120, state="disabled")
        self.btn_future2.grid(row=0, column=7, padx=(10, 0), pady=6)
        ...
        # ===== Painel direito inferior: paleta + preview + status =====
    ...
    def _build_palette(self) -> None:
        # layout com mais linhas “de verdade”: 2 linhas x 8 colunas (16 cores)
    ...
    def _apply_brush_in_mode(self, x: int, y: int, value: int) -> None:
        ...
        for dx, dy in pts:
            ...
            if 0 <= xx < w and 0 <= yy < h:
                self._apply_point_in_mode(xx, yy, value, set_color=True)
    ...
    def _apply_eraser_in_mode(self, x: int, y: int) -> None:
        ...
        for dx, dy in pts:
            ...
            yy = y + dy
            if 0 <= xx < w and 0 <= yy < h:
                self._apply_point_in_mode(xx, yy, 0, set_color=False)
    ...
    def _commit_tool_shape(self, start: Tuple[int, int], end: Tuple[int, int], value: int) -> None:
        ...
        for x, y in pts:
            if 0 <= x < w and 0 <= y < h:
                self._apply_point_in_mode(x, y, value, set_color=True)
