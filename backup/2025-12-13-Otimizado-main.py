...
class SpriteEditorApp(ctk.CTk):
    def __init__(self) -> None:
        ...
        # ============================
        # Redraw scheduling (OTIMIZAÇÃO)
        # ============================
        self._redraw_scheduled: bool = False
        self._pending_redraw_editor: bool = False
        self._pending_redraw_preview: bool = False
        self._pending_thumb_indices: set[int] = set()
    ...
    def _schedule_redraw(
        ...
        *,
        editor: bool = True,
        preview: bool = True,
        thumbs: Optional[List[int]] = None,
    ) -> None:
        """Agrupa múltiplos pedidos de redraw em 1 só (reduz CPU/GDI)."""
        if editor:
            self._pending_redraw_editor = True
        if preview:
            self._pending_redraw_preview = True
        if thumbs:
            self._pending_thumb_indices.update(int(i) for i in thumbs)

        if self._redraw_scheduled:
            return
        self._redraw_scheduled = True
        self.after_idle(self._flush_scheduled_redraw)

    def _flush_scheduled_redraw(self) -> None:
        self._redraw_scheduled = False

        do_editor = self._pending_redraw_editor
        do_preview = self._pending_redraw_preview
        thumbs = list(self._pending_thumb_indices)

        self._pending_redraw_editor = False
        self._pending_redraw_preview = False
        self._pending_thumb_indices.clear()
        ...
        if thumbs:
            self._redraw_thumbnails(indices=thumbs)

    def _affected_sprite_indices_for_pixels(self) -> List[int]:
        """
        Retorna quais sprites podem ter sido alterados no modo atual (para miniaturas).
        single: 1
        overlay: 1 (camada ativa)
        2x2: 4 (bloco)
        """
        return self._get_target_sprite_indices_for_transform()
    ...
    def _build_layout(self) -> None:
        ...
        self.eraser_shape_menu = ctk.CTkOptionMenu(
            er_box, values=["Quadrado", "Redondo"], command=self._on_eraser_shape_changed, width=110
        )
        self.eraser_shape_menu.grid(row=0, column=0, padx=4, pady=6, sticky="ew")
        self.eraser_shape_menu.set(self.eraser_shape.get())

        self.eraser_size_menu = ctk.CTkOptionMenu(
            er_box, values=["1", "2", "3", "4"], command=self._on_eraser_size_changed, width=70
        )
    ...
    def _set_color(self, idx: int) -> None:
        ...
        self._schedule_redraw(editor=True, preview=True, thumbs=self._get_target_sprite_indices_for_color())
    ...
    def _on_mirror_changed(self) -> None:
        ...
        self._schedule_redraw(editor=True, preview=False, thumbs=None)
    ...
    # ============================
    # Transformações / ações (flip/invert/clear/fill)
    # ============================
    ...
    def _action_flip_h(self) -> None:
        ...
        if self._apply_pixel_transform_in_editor_space("flip_h"):
            self._reset_shape_state()
            self._touch_change()
            self._schedule_redraw(editor=True, preview=True, thumbs=indices)
    ...
    # ============================
    # Pintura / eventos do editor (com redraw agendado)
    # ============================
    def _on_editor_press(self, event: tk.Event, value: int) -> None:
        ...
        if self.tool_mode in ("line", "rect", "rect_fill", "ellipse", "ellipse_fill"):
            ...
            self._clear_tool_preview()
            self._touch_change()
            self._schedule_redraw(editor=True, preview=True, thumbs=self._affected_sprite_indices_for_pixels())
            return
    ...
    def _rebuild_sprite_table(self) -> None:
        ...
        self._redraw_thumbnails(indices=None)

    def _select_sprite(self, idx: int) -> None:
        old = self.selected_sprite_index
        self.selected_sprite_index = idx
        self.status_label.configure(text=f"Sprite selecionado: #{idx}")
        self._schedule_redraw(editor=True, preview=True, thumbs=[old, idx])
    ...
    # ============================
    # Redraw (com miniaturas por índice)
    # ============================
    ...
    def _redraw_thumbnails(self, indices: Optional[List[int]] = None) -> None:
        thumb_scale = THUMB_SCALE
        cell = self.sprite_size * thumb_scale

        if indices is None:
            targets = range(len(self.thumb_canvases))
        else:
            s = {int(i) for i in indices if 0 <= int(i) < len(self.thumb_canvases)}
            targets = sorted(s)

        for idx in targets:
            cv = self.thumb_canvases[idx]
            cv.delete("all")
            sp = self.sprites[idx]
